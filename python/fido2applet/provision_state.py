...
class ProvisionState:
    ...
    def invalidate_from_delete_packages(self) -> None:
        ...
        idx = self.steps.index("delete_packages")
    ...
    def reset_from_step(self, step_id: str) -> None:
        ...
        drop = set(self.steps[idx:])
        if step_id in ("install_ndef", "install_fido"):
            # Reinstalling applets must delete old packages first (gp --load alone
            # does not replace bytecode if the package is already on-card).
            drop.add("delete_packages")
        self.completed_steps = [s for s in self.completed_steps if s not in drop]
        if step_id in (
            "install_applets",
            "install_ndef",
    ...
    def run_step(
        ...
        *,
        ...
        if not force and self.state.is_complete(step_id):
            ...
            return None
        if step_id in ("install_ndef", "install_fido", "install_applets") and not self.dry_run:
            # Fresh applet bytecode wipes NdefApplet key material; makeCredential must run again.
            self.state.invalidate_credential_steps()
        try:
            result = fn()
            if not self.dry_run:
                extras: dict[str, Any] = {}
